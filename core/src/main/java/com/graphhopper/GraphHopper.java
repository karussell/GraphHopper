...
public class GraphHopper implements GraphHopperAPI
{
    ...
    protected List<Path> getPaths( GHRequest request, GHResponse rsp )
    {
        ...
        int altMax = request.getHints().getInt("alternatives.max", 0);
        double altMaxShare = request.getHints().getDouble("alternatives.max_share", 0.7);
        ...
        if (!chEnabled && rtMax > 0)
        {
            // TODO currently we need to trigger round trip in the UI via providing two points
//            if (points.size() > 1)
//            {
//                rsp.addError(new IllegalArgumentException("Currently rountrips can be calculated with only one point"));
//                return Collections.emptyList();
//            }
